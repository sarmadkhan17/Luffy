#!/usr/bin/env python3
"""RNG-free deterministic sharding / resume / merge layer for the M3.2 Scheme D validation (bundle v3).

This file constructs no random generator.  Production world evaluation goes through the bundle-v3 runner's own
``SeedRegistry`` (which needs a verified owner authorization); the layer itself only partitions, stores, verifies and
merges.  The v3 bundle, the FastMetric statistic and the protocol are used unchanged.

Sharding scheme
  * Worlds are numbered in ``cell_plan()`` order (cell 0 world 0 .. last cell last world; 75,000 worlds in production).
  * A shard is a contiguous world range inside ONE cell: ``[k*shard_worlds, min((k+1)*shard_worlds, cell.worlds))``.
    Shard ids are sequential in plan order.  The partition is a pure function of (plan, shard_worlds), independent of
    the worker count, so every world belongs to exactly one shard and shards are the resume unit.
  * Every world's randomness is keyed by its own seed tuple, so a shard's output does not depend on which process
    computes it, or in which order or after how many restarts.

Storage (all under ``--out``)
  RUN_MANIFEST.json                    plan, shard table and every binding (bundle / authorization / runner / layer /
                                       environment lock / plan / draws); published exclusively, compared by every worker
  shards/shard_NNNNNN.receipts.jsonl.partial   append-only, one fsynced canonical world receipt per line (resume unit)
  shards/shard_NNNNNN.receipts.jsonl           finalised by an exclusive hard link (never overwritten)
  shards/shard_NNNNNN.complete.json            completion receipt: checksums + bindings, exclusive (never overwritten)
  shards/shard_NNNNNN.lock                     flock guard against a duplicate worker on the same shard
  events/worker_NNNN.jsonl                     structured reason-coded events
  provenance/prov_<sha16>.json                 informational host/CPU/BLAS record per distinct worker environment
  merge.lock                                   flock: exactly one merger at a time (others exit with a reason code)
  merged.staging.<host>.<pid>.<ns>.<n>/        one private staging directory per merger, never touched by another
  merged/                                      cell receipts + run_result.json (byte-identical to the sequential runner)
                                               + MERGE_MANIFEST.json.  Publication order: fsync every staged file, fsync
                                               staging dir, no-replace atomic rename, fsync published dir and its parent,
                                               verify, and only then publish MERGE_MANIFEST.json (the commit marker).

Commands: init | work | status | audit | merge.  Production needs ``--authorization`` (a validation authorization that
also carries ``shard_layer_sha256`` and ``shard_worlds``); ``--fixture`` uses RNG-free arithmetic stand-ins and is
non-inferential by construction.
"""
...
SCHEMA_BINDINGS = "m3.2-scheme-d-shard-bindings.v1"
SCHEMA_RUN = "m3.2-scheme-d-shard-run-manifest.v1"
SCHEMA_SHARD = "m3.2-scheme-d-shard-completion.v1"
SCHEMA_MERGE = "m3.2-scheme-d-shard-merge-manifest.v2"
SCHEMA_PROVENANCE = "m3.2-scheme-d-shard-provenance.v1"
...
def runner():
    """The pinned bundle-v3 runner module (imported lazily; importing it constructs no generator)."""
    if "module" not in _RUNNER:
        sys.path.insert(0, str(BUNDLE / "runner"))
        import scheme_d_runner
        _RUNNER["module"] = scheme_d_runner
...
def expected_tuples(cell: dict, world: int) -> dict:
    """Seed tuples exactly as ``run_cell`` builds them (a pure function of the cell and the world index)."""
    R = runner()
    dgp = cell["dgp"] if cell["kind"] == "null" else cell["scenario"]
    streams = ("data", "membership", "permutation") + (("injection",) if cell["scenario"] else ())
    return {n: [R.MASTER_SEED, cell["phase"], cell["correlation"], dgp, world, R.STREAM[n]] for n in streams}


def make_evaluator(registry, draws: int):
    """Per-world evaluation, identical in structure and argument order to ``scheme_d_runner.run_cell``'s loop body.
    ...
    def evaluate(cell: dict, w: int) -> dict:
        ...
        w_dgp = cell["dgp"] if cell["kind"] == "null" else cell["scenario"]
        if cell["name"] not in truth_cache:
            truth_cache[cell["name"]] = R.load_truth(cell["scenario"], cell["dgp"] if cell["kind"] == "null" else None, c)
        truth = truth_cache[cell["name"]]
        injected = R.V.INJECTIONS[cell["scenario"]] if cell["scenario"] else ()
        tuples = expected_tuples(cell, w)
        world = R.generate_world(1 if cell["kind"] == "power" else cell["dgp"], c, w,
...
def bootstrap_provider(registry):
    """Mirror of ``run_validation``: a bootstrap stream (phase 30, stream 4) only for FDR power cells."""
    R = runner()
...
def production_context(authorization_path: Path, shard_worlds: int) -> Context:
    ...
    measured benchmark, this exact shard layer and shard size) and the FastMetric gate must all verify first."""
    R = runner()
