...
def config() -> dict:
    return {
        "schema": "m3.2-scheme-d-validation-config.v3",
        "bundle_revision": 3,
        "supersedes": {"bundle_manifest": V2_MANIFEST, "bundle_sha256": V2_SHA256,
                       "earlier": [{"bundle_manifest": V1_MANIFEST, "bundle_sha256": V1_SHA256}]},
        "v3_changes": ["statistic path = exact-equivalent FastMetric behind a hash/receipt gate",
                       "runner verifies pinned inputs (incl. the reference and optimized sources) on every RNG mode",
                       "phase-90 benchmark core: reference-vs-fast bit identity on the generated world, fast throughput, projection",
                       "validation authorization must carry a measured fast-benchmark runtime and receipt hash"],
        ...
        "owner_decisions": {
            ...
            "decisions": [
                {"id": "I1", "decision": "dependence group = block index; episode = unique row/episode identity",
                 "implementation": "unchanged from v1", "changed_vs_v1": False, "changed_vs_v2": False},
                ...
                {"id": "I3", "decision": "hypothesis-level refusal; a world with zero tested hypotheses is a refused world, excluded from "
                 ...
                 "changed_vs_v1": True, "changed_vs_v2": False},
                {"id": "I4", "decision": "degenerate = the hypothesis outcome column is constant; an observed zero lift is valid evidence and is never refused",
                 "implementation": "unchanged from v1 (constant-column refusal only); recorded in the addendum", "changed_vs_v1": False, "changed_vs_v2": False},
                ...
                {"id": "I7", "decision": "exact integer BH comparison", "implementation": "unchanged from v1 (exact_bh)", "changed_vs_v1": False, "changed_vs_v2": False},
                {"id": "I8", "decision": "reference _metric path only for the phase-90 benchmark; full-run optimized implementation needs the "
                 ...
                 "changed_vs_v1": False, "changed_vs_v2": True},
...
def equivalence_receipt() -> dict:
    ...
    return {"schema": "m3.2-fast-metric-equivalence-receipt.v1", "result": "PASS_0_MISMATCHES",
            ...
            "reproduction": "verify_bundle.py reruns the complete suite and compares every deterministic field"}
...
def write_manifest() -> None:
    ...
    manifest = {
        "schema": "m3.2-scheme-d-validation-bundle-manifest.v3", "bundle_revision": 3,
        "supersedes": {"bundle_manifest": V2_MANIFEST, "bundle_sha256": V2_SHA256,
                       "v2_manifest_file_sha256": sha(ROOT / V2_MANIFEST),
                       "earlier": [{"bundle_manifest": V1_MANIFEST, "bundle_sha256": V1_SHA256,
                                    "manifest_file_sha256": sha(ROOT / V1_MANIFEST)}]},
