#!/usr/bin/env python3
"""Independent verifier for the RNG-free M3.2 Scheme D validation bundle, REVISION 2.
...
def check_git_and_hashes(report: dict) -> None:
    ...
    need(manifest["schema"] == "m3.2-scheme-d-validation-bundle-manifest.v2" and manifest["bundle_revision"] == 2, "manifest schema")
    need(manifest["supersedes"]["bundle_sha256"] == V1_SHA256 and manifest["supersedes"]["v1_manifest_file_sha256"] == V1_SHA256
         == sha(ROOT / V1_DIR / "BUNDLE_MANIFEST.json"), "v1 supersession record")
    ...
    listed = {str(Path(k).relative_to(V2_DIR)): v
...
def check_config(report: dict) -> None:
    ...
    a = c["authority"]
    need(c["schema"] == "m3.2-scheme-d-validation-config.v2" and c["bundle_revision"] == 2
         and c["supersedes"] == {"bundle_manifest": V1_DIR + "/BUNDLE_MANIFEST.json", "bundle_sha256": V1_SHA256}, "config revision")
...
def check_preflight(report: dict) -> None:
    ...
    need(all(r["result"] == "PASS" for r in recorded["results"]) and recorded["checks_total"] == len(recorded["results"]) == 27,
...
def main() -> int:
    ...
    for step in (check_git_and_hashes, check_tables, check_psd, check_config, check_environment,
                 check_isolation_and_rng, check_v2_semantics, check_preflight):
        step(report)
    report["result"] = "PASS"
