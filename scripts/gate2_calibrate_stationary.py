"""Offline experimental stationary/HAC gate-2 calibration. Generated trades only; never admission.
...
SCHEMA = "luffy.gate2.synthetic.stationary.v1"
...
SCENARIOS = ("iid", "shared_factor", "long_memory", "regime_switching",
             ...
             "spillover_1", "spillover_2", "regime_240", "regime_480", "regime_480_long")
...
def generate(rng, scenario, blocks=12, trades_per_block=20, effect=(0., 0.)):
    ...
    dwell = 240 if scenario == "regime_240" else 480 if scenario.startswith("regime_480") else 120
    regime = scenario == "regime_switching" or scenario.startswith("regime_")
    days = blocks*30
    ...
    for d in range(1, nday):
        state[d] = -state[d-1] if rng.random() < 1/dwell else state[d-1]
    if regime:
        factor = state
    label_factor = (fractional(rng, nday) if scenario == 'long_memory' else
                    persistent(rng, nday, .995**(120/dwell) if regime else .8))
...
def studentized_moments(s, run_blocks=4.):
    """Paired Bartlett HAC SE for the difference of trade-weighted ratios.
    ...
    """
    if not np.isfinite(run_blocks) or run_blocks < 1:
        raise ValueError("run_blocks must be finite and >=1")
    k = s.shape[-4]
    total = s.sum(axis=-4)
    ...
    with np.errstate(divide='ignore', invalid='ignore', over='ignore'):
        ...
        quadratic = np.sum(paired**2, axis=-2)
        for lag in range(1, min(k, math.ceil(run_blocks))):
            quadratic += 2*(1-lag/run_blocks)*np.sum(
                paired[..., lag:, :]*paired[..., :-lag, :], axis=-2)
        # Bartlett is PSD in exact arithmetic. Negative/nonfinite values
        # refuse; never conceal a materially negative estimate by clipping.
        variance = (k/max(k-1, 1))*quadratic
        se = np.sqrt(variance)
        t = delta/se
...
def stationary_indices(rng, draws, k, run_blocks=4.):
    """Circular stationary bootstrap; uniform restarts, geometric run lengths."""
    if draws < 1 or k < 1 or not np.isfinite(run_blocks) or run_blocks < 1:
        raise ValueError("invalid stationary sampler parameters")
    starts = rng.integers(k, size=(draws, k))
    restart = rng.random((draws, k)) < 1/run_blocks
    ix = starts.copy()
    for j in range(1, k):
        ix[:, j] = np.where(restart[:, j], starts[:, j], (ix[:, j-1]+1) % k)
    return ix


def bootstrap(c, draws, rng, block_days=30, decision_alpha=None):
    s, counts = summaries(c, block_days)
    run_blocks = max(1., 120./block_days)
    delta, se, observed, valid = studentized_moments(s, run_blocks)
    extreme = np.zeros(2, dtype=int)
    degenerate = np.zeros(2, dtype=int)
    k = len(s)
    generated = 0
    for start in range(0, draws, 256):
        ix = stationary_indices(rng, min(256, draws-start), k, run_blocks)
        d, se_b, _, ok = studentized_moments(s[ix], run_blocks)
        with np.errstate(divide='ignore', invalid='ignore', over='ignore'):
...
def make_plan(seed=20260918, smoke=False, cap=10000):
    ...
    return {"schema": SCHEMA, "seed": seed, "smoke": smoke, "alpha": .1, "w0": .05,
            ...
            "statistic": "stationary block bootstrap-t with paired Bartlett HAC SE",
            "mean_run_days": 120, "hac_kernel": "Bartlett h < 120/block_days",
            "negative_variance_policy": "refuse; no clipping",
            "sampler": "uniform restarts p=block_days/120; circular continuation",
            "challenge_regime_dwell_days": [240, 480],
            "challenge_label_phi": ".995**(120/dwell_days)",
            "scenario_blocks": {"regime_480_long": 48},
...
def null_cell(plan, scenario, cell):
    ...
    for _ in range(plan['repetitions']):
        c = generate(rng, scenario, 12 if scenario == 'heavy_tail_floor' else plan['scenario_blocks'].get(scenario, plan['null_blocks']),
...
def main(argv=None):
    ...
    source_files = [Path(__file__), ROOT/'trader/research/fdr.py',
                    ROOT/'tests/test_gate2_calibrate_stationary.py',
                    ROOT/'docs/superpowers/specs/2026-09-16-gate2-stationary-calibration.md']
