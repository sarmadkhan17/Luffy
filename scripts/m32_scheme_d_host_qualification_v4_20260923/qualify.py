#!/usr/bin/env python3
"""RNG-free host / filesystem qualification for the v4 M3.2 Scheme D shard layer (bundle v4).

v4-bound sibling of ``scripts/m32_scheme_d_qualification_20260922/qualify.py`` (bundle v3), which stays untouched: the
checks, receipts and combine rules are unchanged; only the bindings (bundle v4, the v4 shard layer and its
``shard_fixture.py``, the measured v4 phase-90 fast-benchmark receipt) and the reference calibration source differ.
The v4 shard layer is loaded under a private ``sys.modules`` key so it never shadows the v3 layer's ``shard_layer``.

No random generator is constructed (worlds are the layer's arithmetic-token fixtures; every probe payload is a hash
ramp), no validation world is generated, no owner authorization is read or created, no benchmark is run, and neither
the bundle nor the protocol is touched.  The tool is bound to the exact bundle-v4, shard-layer and benchmark-receipt
SHA-256 below and fails closed (a FAIL receipt, exit 2) if any differs.
...
ROOT = HERE.parents[1]
LAYER_DIR = ROOT / "scripts/m32_scheme_d_sharding_v4_20260923"
BUNDLE = ROOT / "scripts/m32_scheme_d_validation_bundle_v4_20260922"
_SL_KEY = "m32_v4_host_qualification_shard_layer"


def _load_layer():
    """The v4 shard layer under a private module key (stdlib-only at import; the runner and numpy load lazily)."""
    if _SL_KEY not in sys.modules:
        spec = importlib.util.spec_from_file_location(_SL_KEY, LAYER_DIR / "shard_layer.py")
        module = importlib.util.module_from_spec(spec)
        sys.modules[_SL_KEY] = module
        spec.loader.exec_module(module)
    return sys.modules[_SL_KEY]


SL = _load_layer()

EXPECTED_BUNDLE_SHA256 = "a1e671f581d984521cab43199694f31bec41d0233c4232c1a4e7799114da3023"
EXPECTED_LAYER_SHA256 = "5e322153c62b103d00e9e898b30df4efc2031ea66c7144b79d5037b4d03c2170"
EXPECTED_FIXTURE_SHA256 = "104a516bb26644adbbea18a996ced23230d7ab326eb0b7c57a60637442f76433"
EXPECTED_BENCHMARK_RECEIPT_SHA256 = "f70ace0a02ab6187647ecc33446238cf5ba9dbe97320194ea75f32876972f23e"
EXPECTED_PHASE90_DRAWS_PER_SECOND_PER_CORE = 975.4548174287327
EXPECTED_PHASE90_CPU_HOURS = 16406.782531828467
SCHEMA_RECEIPT = "m3.2-scheme-d-host-qualification-receipt.v4"
SCHEMA_REPORT = "m3.2-scheme-d-qualification-report.v4"
SCHEMA_CALIBRATION = "m3.2-scheme-d-qualification-reference-calibration.v4"
TOOL_PATH = Path(__file__).resolve()
CALIBRATION_PATH = HERE / "reference_calibration.json"
BENCHMARK_DIR = ROOT / "scripts/m32_scheme_d_phase90_fast_benchmark_v4_20260923"
PHASE90 = BENCHMARK_DIR / "benchmark_summary.json"
BENCHMARK_RECEIPT = BENCHMARK_DIR / "out/phase90_benchmark_receipt.json"


def bindings() -> dict:
    return {"bundle_sha256": EXPECTED_BUNDLE_SHA256, "shard_layer_sha256": EXPECTED_LAYER_SHA256,
            "benchmark_receipt_sha256": EXPECTED_BENCHMARK_RECEIPT_SHA256}
...
def benchmark_binding() -> tuple[dict, list[str]]:
    """The measured v4 phase-90 fast-benchmark evidence (summary + receipt hash).  Read only; nothing is re-run and the
    benchmark's authorization file is never opened."""
    problems = []
    receipt_sha = sha256_file(BENCHMARK_RECEIPT) if BENCHMARK_RECEIPT.is_file() else None
    summary = json.loads(PHASE90.read_text()) if PHASE90.is_file() else {}
    if receipt_sha != EXPECTED_BENCHMARK_RECEIPT_SHA256:
        problems.append("benchmark_receipt_sha_mismatch")
    if summary.get("benchmark_receipt_sha256") != EXPECTED_BENCHMARK_RECEIPT_SHA256:
        problems.append("benchmark_summary_receipt_sha_mismatch")
    if summary.get("bundle_sha256") != EXPECTED_BUNDLE_SHA256 or summary.get("shard_layer_sha256") != EXPECTED_LAYER_SHA256:
        problems.append("benchmark_summary_bound_to_other_bundle_or_layer")
    if summary.get("draws_per_second_per_core") != EXPECTED_PHASE90_DRAWS_PER_SECOND_PER_CORE \
            or summary.get("cpu_hours") != EXPECTED_PHASE90_CPU_HOURS:
        problems.append("benchmark_summary_measurement_differs")
    if summary.get("bitwise_identical") is not True or summary.get("validation_authorized") is not False \
            or summary.get("inferential_validation_run") is not False:
        problems.append("benchmark_summary_attestation_unexpected")
    detail = {"benchmark_receipt_sha256": receipt_sha, "expected_benchmark_receipt_sha256": EXPECTED_BENCHMARK_RECEIPT_SHA256,
              "benchmark_summary_sha256": sha256_file(PHASE90) if PHASE90.is_file() else None,
              "phase90_draws_per_second_per_core": summary.get("draws_per_second_per_core"),
              "phase90_projection_cpu_hours": summary.get("cpu_hours")}
...
def check_bindings(ctx) -> dict:
    ...
    fixture_sha = sha256_file(LAYER_DIR / "shard_fixture.py")
    if fixture_sha != EXPECTED_FIXTURE_SHA256 or manifest.get("files", {}).get("shard_fixture", {}).get("sha256") != fixture_sha:
        problems.append("shard_fixture_sha_mismatch")
    bench, bench_problems = benchmark_binding()
    ...
    detail = {"bundle_sha256": bundle_sha, "shard_layer_sha256": layer_sha, "expected_bundle_sha256": EXPECTED_BUNDLE_SHA256,
              "expected_shard_layer_sha256": EXPECTED_LAYER_SHA256, "fixture_sha256": fixture_sha,
              "tool_sha256": sha256_file(TOOL_PATH), **bench}
    if problems:
...
def calibration_binding_problems(cal: dict) -> list[str]:
    """A calibration only scales the v4 phase-90 measurement it was recorded against."""
    problems = []
    if cal.get("schema") != SCHEMA_CALIBRATION:
        problems.append("calibration_schema_mismatch")
    if cal.get("bindings") != bindings():
        problems.append("calibration_bindings_mismatch")
    if cal.get("phase90_draws_per_second_per_core") != EXPECTED_PHASE90_DRAWS_PER_SECOND_PER_CORE \
            or cal.get("phase90_projection_cpu_hours") != EXPECTED_PHASE90_CPU_HOURS:
        problems.append("calibration_phase90_measurement_mismatch")
    return problems
...
def project_throughput(per_draw: float, draws: int) -> dict:
    ...
    problems = calibration_binding_problems(cal)
    if problems:
        return {"calibrated": False, "reason": "reference_calibration_not_bound_to_v4_benchmark", "problems": problems}
...
def combine(receipts: list[dict], invalid: list[dict]) -> dict:
    ...
    return {"schema": SCHEMA_REPORT, "bindings": bindings(),
            ...
            "authorization_prerequisites_outside_this_tool": [
                "v4-bound owner validation authorization with shard_layer_sha256, shard_worlds, runtime_estimate_cpu_hours and "
                "fast_benchmark_receipt_sha256 bound (the measured v4 benchmark receipt is bound here as evidence only)",
                "target hosts running this tool and the peer flock protocol on the real shared mount"],
...
def cmd_calibrate(a) -> int:
    bench, problems = benchmark_binding()
    if problems:
        print(f"fail closed: v4 benchmark binding: {problems}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        shutil.rmtree(work, ignore_errors=True)
    rates = [p["draws_per_second"] for p in procs]
    dps = bench["phase90_draws_per_second_per_core"]
    cal = {"schema": SCHEMA_CALIBRATION, "bindings": bindings(), "reference_host": socket.gethostname(), "processes": a.processes,
           "draws": CALIBRATION_DRAWS, "process_draws_per_second": rates, "proxy_draws_per_second": median(rates),
           "proxy_seconds_per_draw_end_to_end": 1.0 / median(rates),
           "phase90_seconds_per_draw_end_to_end": 1.0 / dps, "phase90_draws_per_second_per_core": dps,
           "phase90_projection_cpu_hours": bench["phase90_projection_cpu_hours"],
           "phase90_summary_sha256": bench["benchmark_summary_sha256"],
           "phase90_benchmark_receipt_sha256": bench["benchmark_receipt_sha256"], "load_average_1m": os.getloadavg()[0],
           "process_spread_max_over_min": max(rates) / min(rates),
           "note": "median across fresh single-worker processes of the deterministic token world (v4 runner + v4 fixture); "
                   "slower per draw than the measured v4 phase-90 world, used only as a ratio; no benchmark was run"}
