"""RNG-free, non-inferential fixtures for the M3.2 v4 shard layer.

Copied unchanged in behaviour from ``scripts/m32_scheme_d_sharding_20260921/shard_fixture.py`` (bundle v3), which
stays untouched; this file lives under the v4 sharding directory and is loaded by ``shard_layer.py`` under a
private ``sys.modules`` key (``fixture_module()``) so it never collides with v3's same-named file.
...
constructs a numpy/python generator.  Worlds, receipts and statistics below are produced by the *real* bundle-v4
runner code (``generate_world``, ``apply_injection``, ``world_receipt`` with the exact-equivalent ``FastMetric``)
fed with these tokens; they carry no statistical meaning and are never a validation world.

``FIXTURE_CELLS`` includes ``P1/C0``, which bundle v4's ``cell_plan()`` tags ``kind="perturbed_null"`` (the
pre-RNG correction): every fixture run below already exercises the perturbed_null dispatch and acceptance path,
not just the power path.
"""
