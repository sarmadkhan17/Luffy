...
Bundle revision 4: applies the 2026-09-20 pre-RNG correction (protocol correction section 2-3).  P1 (scenario 101)
and P3 (scenario 103) are true-null perturbed-calibration scenarios, not power scenarios: ``cell_plan()`` tags
their ten C0-C4 cells ``kind="perturbed_null"``.  World generation and injection dispatch are unchanged and still
occur for every scenario (dispatch is keyed by ``scenario``, not ``kind``); only the acceptance-side evaluation in
``acceptance.evaluate_cell`` differs for ``perturbed_null`` cells (see ``runner/acceptance.py``).  No bootstrap
(stream 4) generator is constructed for P1/P3: they are outside ``acceptance.FDR_SCENARIOS``.
...
# scenarios whose cell_plan() kind is "perturbed_null" instead of "power" (protocol correction section 2-3)
...
REQUIRED_PREFLIGHT_CHECKS = ("fast_metric_hash_binding", "equivalence_receipt_recorded",
                             ...
                             # v4 pre-RNG correction (protocol correction section 2-3): the statistic gate must
                             # not open unless the corrected P1/P3 role/gate checks were recorded PASS.
                             "power_rules_and_fdr_scenarios_exclude_p1_p3", "p1_p3_role_and_cell_kind_and_truth")
...
def apply_injection(world: V.SyntheticWorld, scenario: int, injection_rng) -> V.SyntheticWorld:
    ...
    Dispatch is keyed by ``scenario`` alone (not ``kind``): P1 (101) and P3 (103) still receive their
    injection exactly as before the pre-RNG correction, since only the acceptance-side classification
    of the resulting cell changed, not the generated world.
    """
...
def cell_plan() -> list[dict]:
    """Bundle revision 4: the P1/P3 x C0-C4 cells are tagged ``kind="perturbed_null"`` (protocol correction
    section 2-3); every other scenario cell keeps ``kind="power"``.  World generation is unaffected: dgp
    stays 1 (base N1) and injection is applied for every scenario cell, keyed by ``scenario`` alone.
    """
    plan = [{"kind": "null", "phase": 10, "dgp": d, "scenario": None, "correlation": c, "worlds": 2000,
             "name": f"N{d}/C{c}"} for d in NULL_DGP for c in range(5)]
    plan += [{"kind": "perturbed_null" if s in PERTURBED_NULL_SCENARIOS else "power", "phase": 20, "dgp": 1,
              "scenario": s, "correlation": c, "worlds": 1000, "name": f"P{s - 100}/C{c}"}
             for s in SCENARIOS for c in range(5)]
    return plan


def run_cell(cell: dict, registry: SeedRegistry, out_dir: Path, draws: int) -> list[dict]:
    """Dispatch (base dgp = N1 and injection) is keyed by ``cell["scenario"] is not None``, matching
    ``apply_injection``'s own scenario-only dispatch: every non-null cell (power and, since bundle revision 4,
    perturbed_null) carries a scenario and is generated/injected identically; only ``kind`` -- consumed solely by
    ``acceptance.evaluate_cell`` -- differs for P1/P3.
    """
    has_scenario = cell["scenario"] is not None
    phase, c, w_dgp = cell["phase"], cell["correlation"], cell["scenario"] if has_scenario else cell["dgp"]
    truth = load_truth(cell["scenario"], cell["dgp"] if not has_scenario else None, c)
    ...
    with path.open("x", encoding="utf-8") as fh:               # exclusive create
        for w in range(cell["worlds"]):
            tuples = {n: [MASTER_SEED, phase, c, w_dgp, w, STREAM[n]]
                      for n in ("data", "membership", "permutation") + (("injection",) if has_scenario else ())}
            world = generate_world(1 if has_scenario else cell["dgp"], c, w,
