#!/usr/bin/env python3
"""Independent verifier for the RNG-free M3.2 Scheme D validation bundle, REVISION 4.

Revision 4 applies the 2026-09-20 pre-RNG correction (protocol correction section 2-3): P1 (scenario 101) and
P3 (scenario 103) are corrected from power scenarios to true-null perturbed-calibration scenarios.  This verifier
re-derives the corrected role table, statically confirms ``acceptance.POWER_RULES``/``acceptance.FDR_SCENARIOS``
exclude 101/103, independently re-reads the frozen (unchanged) truth package to confirm every P1/P3 hypothesis is
true_null across C0-C4, and probes ``cell_plan()``/``evaluate_cell`` in an isolated subprocess.

Unlike v3's ``check_equivalence_reproduction``, this verifier does NOT replay the full (7,592,064-comparison)
FastMetric equivalence suite: that binding was already established and hash-pinned by v3 (frozen, unmodified,
re-checked here by hash) and replaying it is expensive RNG-adjacent work outside this correction's authorized
scope.  ``check_fast_metric_artifacts`` still re-verifies the recorded receipt/report/registry hash bindings and
recomputes the benchmark projection; ``check_independent_equivalence_probe`` still runs its own small self-contained
fixture probe against the production ``_metric``; ``check_preflight`` still reruns the full RNG-free preflight
suite and requires it to match the recorded report exactly (which itself contains the deterministic live
bit-identity check ``fast_metric_live_equivalence_deterministic``).

Imports neither the builder, the runner, the preflight suite nor the pinned harness (except where an isolated
subprocess probe is used, mirroring v3's own GATE_PROBE/V2_PROBE pattern).  Re-derives the membership and
effect-assignment tables, the C0-C4 covariance matrices and PSD proofs, the null MAD and the config's fixed
numeric design from the protocol text (typed here), checks every hash binding (bundle files, pinned inputs,
protocol documents, frozen truth package, git ancestry and tracking of v1/v2/v3), re-checks import isolation in a
clean interpreter and the absence of RNG use, and re-runs the preflight suite in a subprocess, requiring its
report to equal the recorded one.  Fails closed.
"""
...
V4_DIR = "scripts/m32_scheme_d_validation_bundle_v4_20260922"
...
def role_of(code: int) -> str:
    """Protocol correction section 2: P1/P3 are true-null perturbed calibration, not acceptance/power."""
    if code == 107:
        return "sensitivity_only"
    if code in PERTURBED_NULL_SCENARIOS:
        return "calibration"
    return "acceptance"
...
def check_git_and_hashes(report: dict) -> None:
    ...
    need(manifest["schema"] == "m3.2-scheme-d-validation-bundle-manifest.v4" and manifest["bundle_revision"] == 4, "manifest schema")
    sup = manifest["supersedes"]
    need(sup["bundle_sha256"] == V3_SHA256 == sup["v3_manifest_file_sha256"] == sha(ROOT / V3_DIR / "BUNDLE_MANIFEST.json"),
         "v3 supersession record")
    need(sup["earlier"] == [{"bundle_manifest": V2_DIR + "/BUNDLE_MANIFEST.json", "bundle_sha256": V2_SHA256,
                             "manifest_file_sha256": V2_SHA256}] and sha(ROOT / V2_DIR / "BUNDLE_MANIFEST.json") == V2_SHA256,
    ...
    need(git("diff", "--quiet", V3_COMMIT, "--", V3_DIR).returncode == 0, "v3 bundle was modified")
    ...
    need(manifest["no_rng_no_seed_generation_no_worlds_no_search_no_gate2"] is False, "preparation exception must not be hidden")
    audit = jl("artifacts/preparation_audit.json")
    need(manifest["preparation_audit"] == {"path": V4_DIR + "/artifacts/preparation_audit.json",
         "sha256": sha(HERE / "artifacts/preparation_audit.json"), "owner_review_required": True}, "preparation audit binding")
    need(audit["owner_review_required"] is True and audit["validation_worlds_generated"] == 0,
         "preparation review status")
    report["preparation_audit"] = manifest["preparation_audit"]
    report["whole_preparation_session_rng_free"] = False
    listed = {str(Path(k).relative_to(V4_DIR)): v
    ...
    for rel, h in listed.items():
        ...
        report.setdefault("git_tracked_pending", []).append(rel) if not tracked else None
...
def check_tables(report: dict) -> None:
    ...
    for s in e["scenarios"]:
        ...
        need(s["role"] == role_of(code), f"role {code}")
    ...
    need(per_kind[107]["case_kind"] == 24 and per_kind[107]["persistence"] == 24 and per_kind[107]["continuation"] == 48,
         "P7 balance")
    need({code: role_of(code) for code in targets} == {101: "calibration", 102: "acceptance", 103: "calibration",
                                                        104: "acceptance", 105: "acceptance", 106: "acceptance",
                                                        107: "sensitivity_only"}, "corrected role table")
    report["hypotheses"] = H
...
def check_config(report: dict) -> None:
    ...
    a = c["authority"]
    need(c["schema"] == "m3.2-scheme-d-validation-config.v4" and c["bundle_revision"] == 4
         and c["supersedes"]["bundle_manifest"] == V3_DIR + "/BUNDLE_MANIFEST.json" and c["supersedes"]["bundle_sha256"] == V3_SHA256
         and c["supersedes"]["earlier"] == [{"bundle_manifest": V2_DIR + "/BUNDLE_MANIFEST.json", "bundle_sha256": V2_SHA256}],
         "config revision")
    ...
    need(tuples == {"null": 4 * 5 * 2000 * 3, "power": 7 * 5 * 1000 * 4, "checks_bootstrap": 20, "total": 260_020},
         "tuple counts (checks_bootstrap = |FDR_POWER_SCENARIOS| * 5 correlations, P1/P3 excluded)")
    ...
    need(c["cells"]["null"] == {"count": 20, "worlds_each": 2000} and c["cells"]["power"] == {"count": 25, "worlds_each": 1000}
         and c["cells"]["perturbed_null"] == {"count": 10, "worlds_each": 1000}
         and c["cells"]["worlds_total"] == 20 * 2000 + 25 * 1000 + 10 * 1000 == 75_000, "cells")
    dgp = c["dgp"]
    ...
    need(ac["wilson_z"] == 1.959964 and ac["type_one"] == {"point_max": 0.05, "wilson_upper_max": 0.06}
         ...
         and sorted(ac["fdr"]["scenarios"]) == list(FDR_POWER_SCENARIOS)
    ...
    need("P1_P3" not in ac["power"], "P1/P3 power gate must be removed by the correction")
    need(ac["power"] == {"P2": {"wilson_lower_min": 0.80},
                         "P4": [{"at_least": 1, "min": 0.90}, {"at_least": 4, "min": 0.75}],
                         "P5_P6": [{"at_least": 1, "min": 0.95}, {"at_least": 16, "min": 0.75}]},
         "power rules (P2's original single-target criterion preserved; P1/P3 excluded)")
    need(ac["p1_p3_false_positive"]["point_max"] == 0.05 and ac["p1_p3_false_positive"]["wilson_upper_max"] == 0.06,
         "p1/p3 false-positive gate thresholds")
    ...
    need([x["id"] for x in od["decisions"]] == [f"I{i}" for i in range(1, 9)] + ["class_edge", "correction_2026-09-20"],
         "owner decision ids")
    need({x["id"] for x in od["decisions"] if x["changed_vs_v1"]} == {"I3", "I6", "class_edge", "correction_2026-09-20"},
         "only I3/I6/class-edge/correction changed vs v1")
    need({x["id"] for x in od["decisions"] if x["changed_vs_v2"]} == {"I8", "correction_2026-09-20"}, "only I8/correction changed vs v2")
    need({x["id"] for x in od["decisions"] if x.get("changed_vs_v3")} == {"correction_2026-09-20"}, "only the correction changed vs v3")
...
def check_preflight(report: dict) -> None:
    ...
    need(all(r["result"] == "PASS" for r in recorded["results"]) and recorded["checks_total"] == len(recorded["results"]),
...
def check_fast_metric_artifacts(report: dict) -> None:
    ...
    report["equivalence_suite_replay"] = "not replayed in v4 (see v3 revision 3); hash/receipt binding re-verified above"
...
perturbed = sorted({p["scenario"] for p in plan if p["kind"] == "perturbed_null"})
power = sorted({p["scenario"] for p in plan if p["kind"] == "power"})
out = {"kinds": kinds, "perturbed_null_scenarios": perturbed, "power_scenarios": power,
       "cells": len(plan), "worlds": sum(p["worlds"] for p in plan),
       "power_rules": sorted(A.POWER_RULES), "fdr_scenarios": sorted(A.FDR_SCENARIOS)}
# true-null rejections -> false positive/FDP; zero rejections passes; excessive rejections fails; refusal excluded
...
out["zero_rejections_passes"] = A.evaluate_cell("perturbed_null", 101, calm)["passes"]
excessive = [{**c, "R": 1, "V": 1} if i < 200 else c for i, c in enumerate(calm)]
cell_fail = A.evaluate_cell("perturbed_null", 103, excessive)
out["excessive_rejections_fails"] = (not cell_fail["passes"]) and cell_fail["mean_fdp"] == 0.2
refused = calm[:800] + [{"refused": True, "refused_hypotheses": 0, "R": 0, "V": 0, "true_discoveries": 0,
                         "attribution_errors": 0, "marginal_p": None}] * 200
cell_refused = A.evaluate_cell("perturbed_null", 101, refused)
out["refusal_blocker_trips"] = cell_refused["refusal"]["blocks"] and not cell_refused["passes"] and cell_refused["non_refused"] == 800
out["perturbed_null_gate_keys"] = sorted(A.evaluate_cell("perturbed_null", 101, calm)["gates"])
print(json.dumps(out))
"""


def check_p1_p3_correction(report: dict) -> None:
    """Independent probe of the 2026-09-20 pre-RNG correction: role table, cell_plan kind, POWER_RULES/FDR_SCENARIOS,
    frozen truth tables (re-read directly, not via the runner) and the perturbed_null acceptance gate semantics.
    """
    src = (HERE / "runner" / "acceptance.py").read_text()
    tree = ast.parse(src)
    literals = {}
    for node in ast.walk(tree):
        if isinstance(node, ast.Assign) and len(node.targets) == 1 and isinstance(node.targets[0], ast.Name):
            name = node.targets[0].id
            if name in ("POWER_RULES", "FDR_SCENARIOS", "PERTURBED_NULL_SCENARIOS"):
                literals[name] = ast.literal_eval(node.value)
    need(set(literals.get("POWER_RULES", {})) == {102, 104, 105, 106}, "static POWER_RULES keys")
    need(literals["POWER_RULES"][102] == ((1, 0.80),), "P2's original single-target power criterion preserved")
    need(set(literals.get("FDR_SCENARIOS", ())) == {102, 104, 105, 106}, "static FDR_SCENARIOS")
    need(tuple(literals.get("PERTURBED_NULL_SCENARIOS", ())) == (101, 103), "static PERTURBED_NULL_SCENARIOS")

    truth_pkg = ROOT / "scripts/m32_scheme_d_pre_rng_truth_20260921/package/tables"
    for scenario in (101, 103):
        for corr in range(5):
            recs = json.loads((truth_pkg / f"P{scenario - 100}_C{corr}.json").read_text())["records"]
            need(len(recs) == 384 and all(r["classification"] == "true_null" and r.get("sign") is None for r in recs),
                 f"P{scenario - 100}/C{corr} truth table not all true_null")
    for scenario in (102, 104, 105, 106):
        recs = json.loads((truth_pkg / f"P{scenario - 100}_C0.json").read_text())["records"]
        need(any(r["classification"] == "analytic_non_null" for r in recs), f"P{scenario - 100}/C0 has no non-null rows")

    proc = subprocess.run([sys.executable, "-c", CELL_PLAN_PROBE % str(HERE / "runner")], capture_output=True, text=True,
                          env={**os.environ, "OPENBLAS_NUM_THREADS": "1"})
    need(proc.returncode == 0, f"cell_plan probe failed: {proc.stderr[-400:]}")
    out = json.loads(proc.stdout.strip().splitlines()[-1])
    need(out["kinds"] == {"null": 20, "power": 25, "perturbed_null": 10}, f"cell_plan kind counts: {out['kinds']}")
    need(out["perturbed_null_scenarios"] == [101, 103], "perturbed_null scenarios")
    need(out["power_scenarios"] == [102, 104, 105, 106, 107], "power scenarios (P2,P4,P5,P6,P7)")
    need(out["cells"] == 55 and out["worlds"] == 75_000, "total cells/worlds unchanged")
    need(out["power_rules"] == [102, 104, 105, 106] == out["fdr_scenarios"], "runtime POWER_RULES/FDR_SCENARIOS")
    need(out["zero_rejections_passes"] is True, "zero rejections must pass the P1/P3 Type-I gate")
    need(out["excessive_rejections_fails"] is True, "20% rejecting must fail the P1/P3 Type-I gate with mean FDP 0.2")
    need(out["refusal_blocker_trips"] is True, "20% refused worlds must trip the unchanged refusal blocker")
    need(out["perturbed_null_gate_keys"] == ["refusal_blocker_absent", "type_one"],
         "perturbed_null cells must carry no power/fdr/marginal_calibration gate")
    report["p1_p3_correction"] = {"cell_kinds": out["kinds"], "power_rules": out["power_rules"],
                                  "truth_tables_checked": 384 * (2 * 5 + 4), "gate_keys": out["perturbed_null_gate_keys"]}
...
def main() -> int:
    report: dict = {"schema": "m3.2-scheme-d-validation-bundle-verification.v4", "independent_implementation": True,
    ...
    for step in (check_git_and_hashes, check_tables, check_psd, check_config, check_environment,
                 ...
                 check_p1_p3_correction, check_independent_equivalence_probe, check_preflight):
        step(report)
    report["result"] = "PASS"
