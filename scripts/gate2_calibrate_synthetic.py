"""Offline experimental gate-2 calibration. Generated trades only; never admission.
...
SCHEMA = "luffy.gate2.synthetic.v1"
...
def bootstrap(c, draws, rng, block_days=30, decision_alpha=None):
    ...
    delta, valid = contrasts(s.sum(axis=0))
    extreme = np.zeros(2, dtype=int)
    degenerate = np.zeros(2, dtype=int)
    k = len(s)
    # Same resample across both predictions preserves their MC dependence.
    generated = 0
    for start in range(0, draws, 256):
        ix = rng.integers(k, size=(min(256, draws-start), k))
        d, ok = contrasts(s[ix].sum(axis=1))
        degenerate += (~ok).sum(axis=0)
        extreme += ((~ok) | (d-delta >= delta)).sum(axis=0)
        generated += len(ix)
        # Optional computational shortcut, NOT statistical optional stopping:
        # keep the full predeclared denominator. This is a lower bound on
        # the final p. Stop only when every decision is irreversibly a fail.
        # No shortcut can stop on a rejection or reduce outer replications.
    ...
    for h in range(2):
        if not counts[h]['eligible'] or not valid[h] or degenerate[h] > .2*draws:
            p[h] = 1.
    return {"p": p, "delta": delta, "counts": counts, "degenerate": degenerate,
            "generated_draws": generated, "planned_draws": draws,
            "p_is_lower_bound": generated < draws}
...
def gate1_proxy(c, rng, shift=0.):
    """Max of three coupled Gaussian scores, NOT production gate1 calibration.
...
def make_plan(seed=20260916, smoke=False, cap=10000):
    ...
    return {"schema": SCHEMA, "seed": seed, "smoke": smoke, "alpha": .1, "w0": .05,
            ...
            "selection": "max-of-three Gaussian coupling proxy, not production gate1",
...
def stream_cell(plan, model, cell):
    ...
    return {"model": model, "selector": "Gaussian max-of-three coupling PROXY",
