...
def config() -> dict:
    return {
        "schema": "m3.2-scheme-d-validation-config.v1",
        ...
        "authority": {"protocol": PROTOCOL, "protocol_sha256": sha(ROOT / PROTOCOL), "correction": CORRECTION,
                      "correction_sha256": sha(ROOT / CORRECTION), "baseline_commit": BASELINE_COMMIT,
        ...
        "acceptance": {"wilson_z": 1.959964, "type_one": {"point_max": 0.05, "wilson_upper_max": 0.06},
                       ...
                       "p7": "descriptive only"},
        "interpretations_flagged_for_owner_review": [
            {"id": "I1", "topic": "episode and dependence group in synthetic rows",
             "choice": "episode = unique row; dependence group = block index (the protocol's exchangeable unit)",
             "why": "support_accounting falls back to row_id for episodes and to one 'unknown' group, which would refuse every hypothesis"},
            {"id": "I2", "topic": "matched / observed floors and worst-case bound",
             "choice": "matched = matched unique rows; observed = matched rows observed; worst case = sum over matched slots of the "
                       "minimum slot mask across blocks of the slot's stratum; transition/sequence additionally need worst-case observed >= 1",
             "why": "protocol gives the bound in words; metric must be defined for every draw"},
            {"id": "I3", "topic": "blocks below 8 observed unique labels", "choice": "refuses all 384 hypotheses of the world (hypothesis-level)",
             "why": "listed under hypothesis-level refusals"},
            {"id": "I4", "topic": "zero or degenerate centered statistic",
             "choice": "refused only when the outcome column is constant over observed rows (constant across the permutation group); "
                       "an exactly zero observed lift is NOT refused",
             "why": "refusing zero lifts would refuse most categorical hypotheses under N1 and trip the 1% blocker"},
            {"id": "I5", "topic": "consumption order inside the data stream",
             "choice": "coverage, jumps, persistence noise, categorical labels", "why": "protocol orders jumps before outcomes only"},
            {"id": "I6", "topic": "marginal calibration when h(w) is refused",
             "choice": "the world stays in the ECDF denominator and its null p counts as not <= t", "why": "refusals never count as evidence"},
            {"id": "I7", "topic": "BH boundary arithmetic", "choice": "exact integer comparison instead of float p <= k*0.05/384",
             "why": "p can equal the threshold exactly (1+X = 100k)"},
            {"id": "I8", "topic": "statistic implementation",
             "choice": "reference path only (production _metric import); no vectorized rewrite, so no equivalence proof is needed or offered",
             "why": "section 3 forbids unproven rewrites"}],
...
def write_manifest() -> None:
    ...
    manifest = {
        "schema": "m3.2-scheme-d-validation-bundle-manifest.v1", "status": "FROZEN_PRE_RUN_BUNDLE_NOT_AUTHORIZED_TO_RUN",
