...
def integrate(fn, tol: arb, *, replay: bool = False) -> acb:
    return acb.integral(
        ...
        deg_limit=100 if not replay else 140,
        eval_limit=500000 if not replay else 900000,
        depth_limit=640 if not replay else 800,
        use_heap=not replay,
    )


def integrate_outer(fn, tol: arb, pieces: int = 32) -> acb:
    """Validated pre-splitting keeps nested analytic balls locally narrow."""
    total = acb(0)
    local_tol = tol/pieces
    for i in range(pieces):
        left = A(Fraction(-T*pieces+2*T*i, pieces))
        right = A(Fraction(-T*pieces+2*T*(i+1), pieces))
        total += acb.integral(fn, left, right, abs_tol=local_tol, rel_tol=local_tol,
                              deg_limit=100, eval_limit=500000,
                              depth_limit=640, use_heap=True)
    return total
...
class BatchIntegrator:
    ...
    def _inner_c3(self, g: acb, count: tuple[int, int], r: arb,
        ...
        if sum(count) == 1:
            injected_kind = 0 if count[0] else 1
            injected_p = member(g, self.ts[injected_kind], Fraction(1, 10), Fraction(9, 10))
            if mode == "base":
                return 1-(1-r)*injected_p
            if target:
                if hkind != injected_kind:
                    raise ArithmeticError("target kind disagrees with singleton factor")
                return r*injected_p if mode == "hit" else 1-injected_p
    ...
    def c3_batch(self, specs: dict[str, dict]) -> dict[str, tuple[arb, arb, arb, arb]]:
        ...
        for scenario, items in sorted(by_scenario.items()):
            ...
            def raw(spec: dict, mode: str) -> arb:
                ...
                print(f"C3 P{scenario} {mode}", flush=True)
                value = integrate_outer(outer, self.tol).real
                if mode == "base":
    ...
    def c4_batch(self, specs: dict[str, dict]) -> dict[str, tuple[arb, arb, arb, arb]]:
        ...
        for scenario, items in sorted(by_scenario.items()):
            ...
            def reduced(mode: str, sec: int, hkind: int, target: bool) -> tuple[arb, arb]:
                ...
                print(f"C4 P{scenario} {key}", flush=True)
                value = integrate_outer(fn, self.tol).real
                cache[key] = value
                return value, tail
