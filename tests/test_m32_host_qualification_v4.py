"""Guard the RNG-free host / filesystem qualification tool bound to the v4 M3.2 shard layer / bundle v4 / v4 benchmark.
...
No test constructs a random generator, generates a validation world, runs a benchmark, or touches an owner authorization.
The v4 tool is imported under a private module name so it never collides with the v3 tool's ``qualify`` module."""
import hashlib
...
QDIR = ROOT / "scripts/m32_scheme_d_host_qualification_v4_20260923"
V3_QDIR = ROOT / "scripts/m32_scheme_d_qualification_20260922"
_KEY = "m32_v4_host_qualify"


def _load():
    if _KEY not in sys.modules:
        spec = importlib.util.spec_from_file_location(_KEY, QDIR / "qualify.py")
        module = importlib.util.module_from_spec(spec)
        sys.modules[_KEY] = module
...
# ------------------------------------------------------------------------------------------------ bindings
def test_bound_hashes_are_the_real_v4_bundle_layer_and_benchmark_hashes():
    assert Q.EXPECTED_BUNDLE_SHA256 == "a1e671f581d984521cab43199694f31bec41d0233c4232c1a4e7799114da3023"
    assert Q.EXPECTED_LAYER_SHA256 == "5e322153c62b103d00e9e898b30df4efc2031ea66c7144b79d5037b4d03c2170"
    assert Q.EXPECTED_BENCHMARK_RECEIPT_SHA256 == "f70ace0a02ab6187647ecc33446238cf5ba9dbe97320194ea75f32876972f23e"
    assert Q.EXPECTED_PHASE90_DRAWS_PER_SECOND_PER_CORE == 975.4548174287327
    assert Q.EXPECTED_PHASE90_CPU_HOURS == 16406.782531828467
    assert Q.LAYER_DIR.name == "m32_scheme_d_sharding_v4_20260923" and Q.BUNDLE.name == "m32_scheme_d_validation_bundle_v4_20260922"
    assert Q.sha256_file(SL.LAYER_PATH) == Q.EXPECTED_LAYER_SHA256 and SL.BUNDLE == Q.BUNDLE
    assert Q.sha256_file(Q.BUNDLE / "BUNDLE_MANIFEST.json") == Q.EXPECTED_BUNDLE_SHA256
    assert Q.sha256_file(Q.BENCHMARK_RECEIPT) == Q.EXPECTED_BENCHMARK_RECEIPT_SHA256
    r = Q.check_bindings({})
    assert r["status"] == "PASS" and r["detail"]["benchmark_receipt_sha256"] == Q.EXPECTED_BENCHMARK_RECEIPT_SHA256
    assert r["detail"]["fixture_sha256"] == Q.EXPECTED_FIXTURE_SHA256


def test_the_v4_layer_and_fixture_are_used_without_shadowing_the_v3_modules():
    assert SL.__name__ != "shard_layer" and Path(SL.__file__).parent == Q.LAYER_DIR
    assert sys.modules.get("shard_layer") is not SL
    F = SL.fixture_module()
    assert Path(F.__file__) == Q.LAYER_DIR / "shard_fixture.py"
    assert Path(SL.runner().__file__) == Q.BUNDLE / "runner/scheme_d_runner.py"
    assert Q.sha256_file(V3_QDIR / "qualify.py") != Q.sha256_file(Q.TOOL_PATH)


@pytest.mark.parametrize("attr", ("EXPECTED_LAYER_SHA256", "EXPECTED_BUNDLE_SHA256", "EXPECTED_BENCHMARK_RECEIPT_SHA256",
                                  "EXPECTED_FIXTURE_SHA256", "EXPECTED_PHASE90_CPU_HOURS"))
def test_a_binding_mismatch_fails_closed_and_skips_every_other_check(tmp_path, monkeypatch, attr):
    monkeypatch.setattr(Q, attr, 1.0 if attr == "EXPECTED_PHASE90_CPU_HOURS" else "0" * 64)
...
def test_reference_calibration_is_bound_to_the_measured_v4_benchmark():
    cal = Q.reference_calibration()
    assert Q.calibration_binding_problems(cal) == [] and cal["schema"] == Q.SCHEMA_CALIBRATION
    assert cal["bindings"] == Q.bindings() and cal["phase90_benchmark_receipt_sha256"] == Q.EXPECTED_BENCHMARK_RECEIPT_SHA256
    assert cal["phase90_draws_per_second_per_core"] == 975.4548174287327 and cal["phase90_projection_cpu_hours"] == 16406.782531828467
    assert abs(cal["phase90_seconds_per_draw_end_to_end"] * 975.4548174287327 - 1.0) < 1e-12
    assert cal["phase90_summary_sha256"] == Q.sha256_file(Q.PHASE90)


@pytest.mark.parametrize("mutate", [lambda c: c.update(schema="m3.2-scheme-d-qualification-reference-calibration.v1"),
                                    lambda c: c["bindings"].update(bundle_sha256="0" * 64),
                                    lambda c: c.update(phase90_projection_cpu_hours=18880.62077238084),
                                    lambda c: c.pop("bindings")])
def test_a_calibration_not_bound_to_the_v4_benchmark_is_never_used(monkeypatch, mutate):
    cal = json.loads(json.dumps(Q.reference_calibration()))
    mutate(cal)
    monkeypatch.setattr(Q, "reference_calibration", lambda: cal)
    r = Q.project_throughput(cal["proxy_seconds_per_draw_end_to_end"], 300)
    assert r["calibrated"] is False and r["reason"] == "reference_calibration_not_bound_to_v4_benchmark"


def test_calibrate_fails_closed_on_a_benchmark_binding_mismatch(tmp_path, monkeypatch):
    monkeypatch.setattr(Q, "EXPECTED_BENCHMARK_RECEIPT_SHA256", "0" * 64)
    monkeypatch.setattr(Q, "single_worker_processes", lambda *a, **k: pytest.fail("calibration must not run"))
    before = Q.sha256_file(Q.CALIBRATION_PATH)
    assert Q.main(["calibrate", "--dir", str(tmp_path)]) == 2 and Q.sha256_file(Q.CALIBRATION_PATH) == before
...
def run_peer(tmp_path, label, peer, boot=None, timeout=60):
    code = ("import sys, json, importlib.util; from pathlib import Path\n"
            "s = importlib.util.spec_from_file_location('m32_v4_host_qualify', %r); Q = importlib.util.module_from_spec(s)\n"
            "sys.modules['m32_v4_host_qualify'] = Q; s.loader.exec_module(Q)\n"
            "%s\nprint(json.dumps(Q.flock_peer(Path(%r), %r, %r, %s), default=str))"
            % (str(QDIR / "qualify.py"), (f"_i=Q.host_identity; Q.host_identity=lambda: {{**_i(), 'boot_id_sha256': {boot!r}}}" if boot else ""),
...
@pytest.mark.parametrize("mutate,problem", [
    ...
    (lambda b: b["bindings"].update(benchmark_receipt_sha256="0" * 64), "bindings_mismatch"),
    (lambda b: b.update(bindings={"bundle_sha256": "eab05d5d5acf1e4d435010b30005c9fda164da1b8c9fc0377502579c7d21dc01",
                                  "shard_layer_sha256": "68195fb6f8548722d3055a00e5892d9d5fdc1caaab0735b3f706e05bc1ab0657"}),
     "bindings_mismatch"),
...
def test_quick_host_run_end_to_end_seals_a_receipt_and_combine_refuses_it_as_a_qualification(tmp_path):
    ...
    assert receipt["bindings"] == Q.bindings() and receipt["benchmark_run"] is False
    assert receipt["schema"] == "m3.2-scheme-d-host-qualification-receipt.v4"
...
def test_tool_constructs_no_random_generator_reads_no_authorization_and_touches_no_protocol_file(tmp_path, monkeypatch):
    ...
    for banned in ("import random", "default_rng", "RandomState", "multiprocessing", "import secrets", "import uuid", "urandom",
                   "SeedRegistry", "R.authorize", "assert_equivalence_gate", "run_mode(", "authorization.json",
                   "bench_m32", "run_benchmark", "production_context"):
    ...
    R = SL.runner()
    for name in ("authorize", "run_mode", "SeedRegistry"):
        monkeypatch.setattr(R, name, trip(f"R.{name}"))
    real_read_text, real_read_bytes = Path.read_text, Path.read_bytes

    def guarded(real):
        def f(self, *a, **k):
            if "authorization" in self.name:
                trips.append(f"read:{self}")
                raise AssertionError(self)
            return real(self, *a, **k)
        return f
    monkeypatch.setattr(Path, "read_text", guarded(real_read_text))
    monkeypatch.setattr(Path, "read_bytes", guarded(real_read_bytes))
    assert Q.check_bindings({})["status"] == "PASS"
    Q.project_throughput(0.001, 300)
    Q.numeric_canary()
    fw = Q.FastWorkload()
    fw.measure(5)
