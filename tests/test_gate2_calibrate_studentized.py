...
PATH = Path(__file__).resolve().parents[1]/'scripts/gate2_calibrate_studentized.py'
spec = importlib.util.spec_from_file_location('gate2_calibrate_studentized', PATH)
...
def literal_moments(c, ix, h):
    ...
    return means[1]-means[0], math.sqrt(len(ix)/(len(ix)-1)*sum(v*v for v in u))
...
def test_whole_block_bootstrap_matches_literal_reference():
    ...
    extreme = np.zeros(2)
    for ix in np.random.default_rng(seed).integers(24, size=(draws, 24)):
...
def test_invalid_resamples_keep_full_denominator_and_threshold():
